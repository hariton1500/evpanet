...
class Abonent {
  ...
  Future<void> getDataForGuidsFormServer() async {
    http.Response _response;
    Map<String, String> _headers = {'token': device};
    String url = '';
    print('[getDataForGuidsFormServer] Start get data from server for guids: [$guids]');
    guids.forEach((guid) async {
      ...
      try {
        ...
        if (answer.runtimeType
            ...
            .startsWith('_InternalLinkedHashMap')) {
          if (Map.from(answer).containsKey('message')) {
            lastApiMessage = Map.from(answer)['message']['userinfo'].toString();
